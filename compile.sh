...
echo "set FILE_NAME ${FILE_NAME}"
BUILDPACK_LIBREOFFICE_PACKAGE=https://s3-eu-west-1.amazonaws.com/libreoffice-heroku-buildpack/${FILE_NAME}
echo "set BUILDPACK_LIBREOFFICE_PACKAGE ${BUILDPACK_LIBREOFFICE_PACKAGE}"
ARCHIVE_NAME=opt/libreoffice${VERSION}
echo "set ARCHIVE_NAME ${ARCHIVE_NAME}"


# LibreOffice Dependencies URL
DEPS_FILE_NAME=libreoffice${VERSION}_x86-64_deps.tar.gz
echo "set DEPS_FILE_NAME ${DEPS_FILE_NAME}"
DEPS_BUILDPACK_LIBREOFFICE_PACKAGE=https://s3-eu-west-1.amazonaws.com/libreoffice-heroku-buildpack/${DEPS_FILE_NAME}
echo "set DEPS_BUILDPACK_LIBREOFFICE_PACKAGE ${DEPS_BUILDPACK_LIBREOFFICE_PACKAGE}"
DEPS_ARCHIVE_NAME=app/vendor/libreoffice/deps
echo "set DEPS_ARCHIVE_NAME ${DEPS_ARCHIVE_NAME}"

mkdir -p $CACHE_DIR
echo "makedir CACHE_DIR ${CACHE_DIR}"
echo "check if ${CACHE_DIR}/${FILE_NAME} does not exist then execute curl on ${BUILDPACK_LIBREOFFICE_PACKAGE} and put it there"
...
echo "check if ${CACHE_DIR}/${DEPS_FILE_NAME} does not exist then execute curl on ${DEPS_BUILDPACK_LIBREOFFICE_PACKAGE} and put it there"
...
echo "makedir CACHE_DIR/ARCHIVE_NAME ${CACHE_DIR}/${ARCHIVE_NAME}"

mkdir -p $BUILD_DIR/vendor/
echo "makedir BUILD_DIR/vendor/ ${BUILD_DIR}/vendor/"
...
#mirror.clarkson.edu/tdf/libreoffice/stable/5.3.5/deb/x86_64/LibreOffice_5.3.5_Linux_x86-64_deb.tar.gz
DEB_DOWNLOAD_URL="http://mirror.clarkson.edu/tdf/libreoffice/stable/5.3.5/deb/x86_64/LibreOffice_5.3.5_Linux_x86-64_deb.tar.gz"
...
archive_name=$(tar tzf LibreOffice_5.3.5_Linux_x86-64_deb.tar.gz | sed -e 's@/.*@@' | uniq)
tar xzf LibreOffice_5.3.5_Linux_x86-64_deb.tar.gz
